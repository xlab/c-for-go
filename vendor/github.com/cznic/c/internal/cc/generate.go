...
func yy() (nm string, err error) {
	y, err := ioutil.TempFile("", "go-generate-")
	if err != nil {
		return "", err
	}
...
func main0() (err error) {
	...
	defer func() {
		if e := os.Remove(y); e != nil && err == nil {
			err = e
		}
	}()
...
func main2() (err error) {
	goCmd := exec.Command("go", "test", "-maxerrors", "-1", "-run", "^Example")
	out, _ := goCmd.CombinedOutput() // Errors are expected and wanted here.
